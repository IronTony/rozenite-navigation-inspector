  ...
  return {
    ...
    children: state.routes.map((route, index) => {
      ...
      if (childState && childState.stale === false) {
        const subtree = buildTreeFromState(
        ...
        return {
          ...
          path:
            (route.path ? normalizeRoutePath(route.path) : undefined) ||
            `/${stripRouteGroups(route.name)}`,
...
export function extractFocusedRoute(state: NavigationState): RouteInfo | null {
  ...
  return {
    ...
    path:
      (focusedRoute.path ? normalizeRoutePath(focusedRoute.path) : undefined) ||
      `/${stripRouteGroups(focusedRoute.name)}`,
...
export function collectRouteNames(
  ...
  for (const routeName of state.routeNames) {
    ...
    if (childState && childState.stale === false) {
      const childNames = collectRouteNames(
        childState as NavigationState,
        fullPath,
      );
