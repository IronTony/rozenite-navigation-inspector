...
function collectRoutesFromRouteNode(
  node: RouteNode,
  parents: string[] = [],
): string[] {
...
function buildSitemapFromRouteNode(
  ...
  if (node.type === 'layout') {
    for (const child of node.children) {
      entries.push(
        ...buildSitemapFromRouteNode(child, visitedRoutes, currentPath),
      );
    }
    return entries;
  }
  ...
  return {
    ...
    getAllRoutes() {
      try {
        const {
          store: routerStore,
        } = require('expo-router/build/global-state/router-store');
